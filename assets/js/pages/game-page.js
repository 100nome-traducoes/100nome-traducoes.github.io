$(document).ready(function() {
    ...
    function initImageLoadingPlaceholders() {
        ...
        window.SiteImagePlaceholders.init({
            selector: '.game-cover, .carousel-slide img, .related-game-image',
    ...
    function initCarousel() {
        ...
        lightboxEl.addEventListener('click', (event) => {
            ...
            if ((Date.now() - lightboxOpenedAt) < 220) return;

            const clickedControl = Boolean(target.closest('.game-lightbox-close, .game-lightbox-nav'));
            const clickedStage = Boolean(target.closest('.game-lightbox-stage'));
            const clickedBackdrop = target === lightboxEl;
            const clickedInsideDialog = Boolean(target.closest('.game-lightbox-dialog'));
            const isTouchUi = window.matchMedia('(hover: none), (pointer: coarse)').matches;
            const shouldClose = isTouchUi
                ? (clickedBackdrop || !clickedInsideDialog)
                : (clickedStage || clickedBackdrop);

            if (!clickedControl && shouldClose) {
                closeLightbox();
            }
        });
        ...
        carouselTrackEl.addEventListener('touchend', e => {
            ...
            const endX = changed ? changed.clientX : touchLastX;
            const endY = changed ? changed.clientY : touchLastY;
            const diff = endX - touchStartX;
            const diffY = endY - touchStartY;
            const isTapIntent = touchMode === null && Math.abs(diff) < 10 && Math.abs(diffY) < 10;
            if (touchMode === 'horizontal' && Math.abs(diff) > 50) {
                goTo(current + (diff < 0 ? 1 : -1));
            } else {
                render();
                resetAuto();
                if (!touchMoved && isTapIntent && touchStartSlideIndex >= 0) {
    ...
    function obterSlugJogoAtual() {
        if (window.SiteUtils?.getGameSlug) {
            return window.SiteUtils.getGameSlug();
        }
        const fromDataset = (
    ...
    function initInstallerPostInstallNudge() {
        ...
        if (!commentsSection || !topNotices) return;
        ...
        card.className = 'game-top-nudge installer-post-nudge';
        ...
        card.innerHTML = `
            ...
            <div class="installer-post-nudge-actions">
                ...
                <a href="${DISCORD_URL}" target="_blank" rel="noopener noreferrer" class="btn btn-secondary btn-discord-primary" data-installer-cta="discord">
