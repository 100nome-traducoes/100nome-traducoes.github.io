...
const { resolveSiteUrl, DISCORD_URL, applyGlobalSiteLinks } = require('./site-config');
...
function getGiscusThemeValue() {
  ...
  return `${SITE_URL}/assets/css/components/giscus-theme.css?v=${ASSET_VERSIONS.giscusThemeCss}`;
}
...
function detectChecksumAlgorithm(checksum) {
  const value = String(checksum || '').trim().toLowerCase();
  if (!value) return 'Checksum';
  if (/^[a-f0-9]{32}$/.test(value)) return 'MD5';
  if (/^[a-f0-9]{40}$/.test(value)) return 'SHA-1';
  if (/^[a-f0-9]{64}$/.test(value)) return 'SHA-256';
  return 'Checksum';
}
...
function resolvePrimaryAction(jogo) {
  ...
  const helpUrl = String(jogo?.primaryActionHelpUrl || DISCORD_URL).trim();
...
function buildDownloadSection(jogo, downloadsData) {
  ...
  <section class="game-section download-section" id="download">
    ...
    <div class="section-content">
      ...
      <p class="download-report-links">
        <a href="${escapeHtml(DISCORD_URL)}" target="_blank" rel="noopener noreferrer"><i class="fab fa-discord"></i> Reportar bug no Discord</a>
      </p>
    </div>
  </section>`;
}
...
function buildCommentsSection(jogo) {
  ...
  <section class="game-section comments-section" id="comentarios">
    ...
    <div class="section-content">
      ...
      <p class="comments-game-context">Estás a comentar a tradução de <strong>${escapeHtml(title)}</strong>.</p>
      ...
      <script src="https://giscus.app/client.js"
        ...
        data-theme="${escapeHtml(getGiscusThemeValue())}"
        data-lang="pt"
  ...
  return (atributos || [])
    ...
    .map(item => `
      <span class="gp-tooltip-wrap">
        <img class="attribute-badge" alt="${escapeHtml(item.alt)}" src="/assets/images/site/atributos/${item.img}">
...
function buildTranslationSection(jogo) {
  ...
  const packageChecksumText = String(jogo.packageChecksum || '').trim();
  const packageChecksumAlgorithm = detectChecksumAlgorithm(packageChecksumText);
  const packageInfoText = [packageSizeText, packageLastModifiedText].filter(Boolean).join(' · ');
  const packageInfoHtml = (packageInfoText || packageChecksumText)
    ? `<div class="gp-info-item">
        <i class="fas fa-box-archive"></i>
        <div>
          <span class="gp-info-label gp-info-label-row">Pacote${packageChecksumText ? ` <button type="button" class="gp-integrity-toggle" data-integrity-toggle aria-expanded="false">Verificar ficheiro</button>` : ''}</span>
          ${(packageInfoText || packageChecksumText) ? `<span class="gp-info-value">${packageInfoText ? escapeHtml(packageInfoText) : 'Checksum disponível'}</span>` : ''}
          ${packageChecksumText ? `
          <div class="gp-integrity-panel" data-integrity-panel hidden>
            <div class="gp-integrity-row">
              <span class="gp-info-value gp-info-value--checksum">${escapeHtml(packageChecksumAlgorithm)}: ${escapeHtml(packageChecksumText)}</span>
              <button type="button" class="gp-integrity-copy" data-copy-hash="${escapeHtml(packageChecksumText)}">Copiar hash</button>
            </div>
            <div class="gp-integrity-row gp-integrity-row--note">
              <span class="gp-integrity-note">Para utilizadores avançados: validar ficheiro descarregado.</span>
            </div>
          </div>` : ''}
        </div>
      </div>`
...
function buildPageHtml(template, jogo, allGames, categoriasPrincipais, downloadsData, header, footer, favicon) {
  ...
  const ogImage = coverPath ? `${SITE_URL}/${coverPath}` : `${SITE_URL}/assets/images/site/logo.png`;
  ...
  return applyGlobalSiteLinks(html);
}
